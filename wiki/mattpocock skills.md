...
> 一句话定义：[[Matt Pocock]] 开源的一套 Claude Code skills（GitHub 170K★/750 万下载），给 AI 编程加上工程纪律——用 grill 盘问对齐需求、用 spec/tickets 拆分跨 session 的工作、用 implement + 内置 code review 兜底质量。是 Claude Code 生态最主流的 AI 工程化工具集。
...
- **/grill-with-docs**：基于一个 idea（可极模糊）盘问，探索代码，把"我想改 X"变成 crisp defensible plan；同时把学到的写入 `context.md` 与 ADR。一次约 20 问。
...
- **/grill-me**：和 grill-with-docs 一样盘问但不写文档（stateless），纯对齐。⚠️ **Matt 本人已在 AI Hero 官方页声明 coding 场景不再推荐 `/grill-me`，改推 `/grill-with-docs`**（`/grill-me` 退为"非编码压力测试/临时轻量对齐"）。演进链与新链路详见 [[grill-me 实战指南]]、[[还在用 grill-with-docs]]。
...
- **仓库 stars 数据各源不一致**：视频录制时 162K、YouTube 描述写 170K；[[MattPocock Skills v1.1 重磅更新]] 记 160K；[[mattpocock skills 秒杀系统实战]] 记"接近 150K"；[[Matt Pocock 后台 research 与主线程并行]] 记"约 16 万（2026-07）"。属时间点不同的正常增长，非矛盾——本页取**最新 170K**（2026-07-26 检索）。
- **skill 数量口径（重要）**：v1.1.0 的 promoted skill 数有两说——[[MattPocock Skills 21 个 skill 工程纪律体系]]（早期写作）称 21 个；[[12 行 vs 689 行：mattpocock skills 与 superpowers 的路线之争]] 基于 GitHub API 源码核对称 **18 个**（12 user-invoked + 6 model-invoked），并指出"21 是早期流传数"。建议以源码核对的 **18** 为准。
...
- **来源**：[[Matt Pocock 完整工作流视频]] · [[mattpocock skills 标准工作流]] · [[MattPocock Skills v1.1 重磅更新]] · [[mattpocock skills 秒杀系统实战]] · [[Matt Pocock skills v1.1 官方更新日志]] · [[Matt Pocock skills Wayfinder 官方文档]] · [[Matt Pocock skills Handoff 官方文档]] · [[MattPocock Skills 21 个 skill 工程纪律体系]] · [[Matt Pocock main flow 五环节]] · [[Matt Pocock 6 个 skill 工程师本能]] · [[Matt Pocock 三个 skill 结构词汇]] · [[Matt Pocock writing-great-skills 八词诊断]] · [[Matt Pocock setup-matt-pocock-skills 配置落地]] · [[Matt Pocock wayfinder handoff 接力协议]] · [[Matt Pocock 三条 on-ramp 分流]] · [[Matt Pocock 后台 research 与主线程并行]] · [[Matt Pocock Prototype 保真度方法论]] · [[grill-me 实战指南]] · [[还在用 grill-with-docs]] · [[diagnosing-bugs 与 tdd]] · [[Matt Pocock 的 Git 护栏]] · [[Matt Pocock 工作流五步主线鸟瞰]]
